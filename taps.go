...
func tapsInvAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		for i := range tap_bevs.Items {
			...
			_, err := db.Exec("INSERT INTO location_beverages(beverage_id, location_id, quantity, inventory, update, active) VALUES ($1, $2, $3, $4, $5, TRUE);", bev.ID, bev.LocationID, bev.Quantity, inventory, cur_time)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				log.Println(err.Error())
				continue
